package com.ocr.myapplication.animation
 ...
 * This allows users to drag the fragment down to dismiss it.
 */
class DragToDismissHelper(
    ...
    data class Config(
        val dragHandleAreaPercentage: Float = 0.2f, // Top percentage of view that acts as drag handle
        val dismissThreshold: Float = 0.3f, // Percentage of screen height to trigger dismiss
        val minVelocityToDismiss: Float = 1000f, // Minimum velocity (px/s) to trigger dismiss
        ...
        rootView.setOnTouchListener { view, event ->
            when (event.action) {
                MotionEvent.ACTION_DOWN -> {
                    // Only start dragging if touch is near the top of the fragment
                    if (event.y < rootView.height * config.dragHandleAreaPercentage) {
                        initialY = event.rawY
                        initialX = event.rawX
                ...
                MotionEvent.ACTION_MOVE -> {
                    if (isDragging) {
                        ...
                        if (newY >= 0) {
                            ...
                            // Gradually fade out as the fragment moves down
                            val alpha = 1.0f - (newY / view.height)
 ...
 */
fun Fragment.enableDragToDismiss(
    rootView: View,
    config: DragToDismissHelper.Config = DragToDismissHelper.Config()
): DragToDismissHelper {
    return DragToDismissHelper(this, rootView, config)
}
